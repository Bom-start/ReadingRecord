...
- 하나 이상의 오브젝트가 만들어져야 하기 때문에 싱글톤에 DI 해주는 방법으론 사용불가.
- 스코프 빈은 프로토타입 빈과 마찬가지로 DL 방식으로 사용해야한다.
- 스코프 빈은 싱글톤에서 일반적인 방법으로 DI하는 것이 불가능하기 때문에 직접 DI 하는대신 스코프 빈에 대한 프록시를 DI하는 방식을 히용할 수 있다.
- 클라이언트에서 스코프 프록시 오브젝트를 실제 스코프 빈처럼 사용하면 프록시에서 현재 스코프에 맞는 빈 오브젝트로 작업을 위임한다.
- 스코프 프록시는 각 요청에 연결된 세션 정보를 참고해서 사용자마다 다른 오브젝트를 사용하게 해준다.
- 스코프 프록시는 실제 스코프 오브젝트를 상속한다.
- 프록시 방식의  DI를 적용하면 스코프 빈이지만 싱글톤 빈을 사용하듯 편하게 쓸수 있다는 장점이 있다. 그러나 주입되는 빈의 스코프를 모르면 코드를 이애하는 것이 어려울수도 있다.
