...
- **컨테이너 인프라 빈을 위한 자바 코드 메타정보**
  ...
  - 자바 코드를 이용한 컨테이너 인프라 빈 등록
    ...
    - @ComponentScan
      ...
      ~~~java
      /* 
      	@Configuration이 붙은 클래스에 @ComponentScan 애노테이션을 추가하면 
      	XML에서 <context:component-scan>을 사용한 것처럼 
      ...
      */
      ...
      /* 
      	마커 클래스나 인터페이스를 이용해 스캔 기반 패키지를 지정할 수도 있다.
      	이 경우 오타가 나더라도 IDE에서 에러를 알려주기 때문에 안전하다.
      */
      @Configuration
