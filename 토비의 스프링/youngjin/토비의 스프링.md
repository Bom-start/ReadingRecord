...
비즈니스 로직을 `upgradeLevelsInternal()`로 분리했다. 하지만 여전히 UserService에 트랜잭션 경계 코드와 비즈니스 로직이 혼합되어 있다. DI를 이용하여 클래스를 분리해보겠다.

현재 관계도는 아래와 같다. 클라이언트에서 Test와 같은 클라이언트에서 UserService를 직접 연결하는 강한 결합관계이다.

![](/Users/Andy/IdeaProjects/ReadingRecord/토비의 스프링/youngjin/resources/aop1.png)

이 관계를 아래처럼 인터페이스를 도입하여 약한 결합관계로 변경할 것이다. 

![](/Users/Andy/IdeaProjects/ReadingRecord/토비의 스프링/youngjin/resources/aop2.png)

기존의 UserService 클래스 이름을 UserServiceImpl로 변경하고, UserService라는 이름의 인터페이스를 생성해서 OCP를 적용한다.
...
원래 UserService는 UserServiceImpl로 이름을 변경하고 UserService를 상속(구현)한다.
...
UserServiceImpl로 변경된 클래스에선 트랜잭션 코드를 완전히 걷어냈다.

트랜잭션 코드는 UserServiceTx로 분리했다.
...
public class UserServiceTx implements UserService {
   ...
   public void setUserService(UserService userService) {
      this.userService = userService;  // UserService를 구현한 다른 구현체를 외부에서 주입받는다
   }
  
   // 외부에서 주입받은 UserService 객체에 모든 기능을 의존시킨다(위임).
   @Override
   public void add(User user) {
...
여기까지 하면 UserServiceImpl은 비즈니스 로직을, UserServiceTx은 외부에서 UserService를 주입받아서 해당 구현체에 모든 기능을 위임한다.

이제 UserServiceTx에 트랜잭션 경계설정을 부여한다.
...
이렇게 UserServiceTx에 트랜잭션 경계설정까지 처리하면, UserService를 구현하는 두 객체가 완성된다. UserService는 비즈니스 로직을 구현, UserServiceTx는 트랜잭션 설정을 구현하도록 분리되었다.
...
이렇게 코드를 트랜잭션 경계설정 코드와 비즈니스 로직 분리는 여러가지 장점이 존재한다.

하나는 비즈니스 로직을 담당하는 UserServiceImpl의 코드를 작성할때는 트랜잭션 같은 기술적인 내용에는 전혀 신경 쓰지 않아도 된다. 트랜잭션의 적용이 필요한지도 신경 쓰지 않아도 된다. 

## Mockito

의존 오브젝트를 필요로 하는 코드를 테스트 하기 위해서는 mock 오브젝트 사용이 필수적이다. 이를 위한 프레임워크가 Mockito이다.

~~~java
@Test
public void mockUpgradeLevels() throws Exception {
   UserServiceImpl useServiceImpl = new UserServiceImpl();
  
   // 다이내믹한 목 오브젝트 생성과 메소드의 리턴값 설정
   // 그리고 DI까지 세 줄이면 충분
...
프록시(Proxy)라는 용어와 디자인 패턴에서 말하는 프록시 패턴은 구분한다. 프록시(Proxy)는 클라이언트와 사용 대상 사이에 대리 역할을 맡은 오브젝트를 두는 방법을 총칭한다면, 프록시 패턴은 사용하는 방법 중에서 타깃에 대한 접근 방법을 제어하려는 목적을 가진 경우이다.

프록시 패턴의 프록시는 타깃의 기능을 변경하지 않고, 타깃에 접근하는 방식을 변경한다. 클라이언트에 타깃에 대한 레퍼런스를 넘거야 하는데, 실제 타깃 오브젝트를 만드는 대신 프록시를 넘겨주는 것이다.
